"""
Donchian Weekly — Rejim-Bazlı Sizing Doğrulama Backtest'i

3 konfigürasyon karşılaştırır:
  A) baseline : %10 pozisyon, max 5 (canlıdaki mevcut ayar)
  B) aggressive: %18 pozisyon, max 6 (sabit agresif)
  C) regime   : ayıda A, boğada B — rejim = BTC haftalık kapanış vs EMA40(haftalık)
                (EMA40 haftalık ≈ EMA200 günlük, klasik trend filtresi)

Veri: Binance 1w mumları doğrudan (canlı bot bots/donchian_live.py ile aynı kaynak).
Sinyaller kapanmış mumlardan (look-ahead yok). Slippage + %0.1 komisyon dahil.

Kullanım (container içinde):
  python tools/donchian_regime_backtest.py --capital 340
"""
import argparse
import json
import time
...
CONFIGS = {
    'A) baseline %10/5': {'bear': (0.10, 5), 'bull': (0.10, 5)},
    'B) agresif  %18/6': {'bear': (0.18, 6), 'bull': (0.18, 6)},
    'C) rejimli 10→18': {'bear': (0.10, 5), 'bull': (0.18, 6)},
}
...
def wilder_atr_series(candles):
    """Her bar için o bar DAHİL ATR (orijinal backtest ile uyumlu)."""
...
def prepare_coin(candles):
    """Bar i için sinyaller: d_high/d_low önceki barlardan (shift 1)."""
    out = {}
...
def btc_regime_map(btc_candles):
    """ts → 'bull'/'bear'. Bar i rejimi = kapanış[i-1] vs EMA40[i-1] (kapanmış hafta)."""
    ...
    for i in range(1, len(btc_candles)):
        warm = i - 1 >= REGIME_EMA  # EMA oturmadan hep bear say
...
def simulate(coin_data, regime_map, cfg, start_ts, initial_cash):
    ...
    for ts in timeline:
        ...
        for sym in list(positions):
            ...
            exit_price = None
            if bar['low'] <= pos['stop']:
                exit_price, reason = pos['stop'] * (1 - slip), 'SL'
            elif bar['d_low'] > 0 and bar['low'] <= bar['d_low']:
                exit_price, reason = bar['d_low'] * (1 - slip), 'donchian'
            if exit_price:
...
def main():
    ...
    parser.add_argument('--capital', type=float, default=340.0)
